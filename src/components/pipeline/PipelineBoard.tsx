...
import { useMoveDeal, usePipeline, usePipelines } from '@/hooks';
import type { DragEndEvent, DragStartEvent } from '@dnd-kit/core';
import { clsx } from 'clsx';
import { Building2, ChevronDown, DollarSign, Plus, Settings } from 'lucide-react';
...
import { Badge, Button, Card, ErrorState, SkeletonKanban, Tooltip } from '@/components/ui';
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-2 flex flex-col gap-2">
        {column.deals.length === 0 ? (
          <div className="flex-1 flex items-center justify-center py-8">
            <p className="text-xs text-text-tertiary text-center">Arraste negócios para cá</p>
          </div>
        ) : (
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        <ChevronDown
          size={18}
          className={clsx('text-text-secondary transition-transform', open && 'rotate-180')}
        />
      </button>
          ...
          <div className="py-1">
            {pipelines.map((pipeline) => (
              <button
                ...
                className={clsx(
                  'w-full px-4 py-2.5 text-left hover:bg-bg-hover transition-colors focus-ring',
              ...
              >
                ...
                {selectedId === pipeline.id && <div className="w-2 h-2 rounded-full bg-primary" />}
              </button>
            ))}
          </div>
          <div className="border-t border-bg-border py-1">
            <Link
              ...
              className={clsx(
                ...
                'flex items-center gap-2 text-text-secondary focus-ring',
              )}
            >
              <Settings size={16} />
              Gerenciar Pipelines
...
export default function PipelineBoard() {
  ...
  const handleDragEnd = useCallback(
    async (event: DragEndEvent) => {
      ...
      const sourceColumn = columns.find((col) => col.deals.some((d) => d.id === activeId));
