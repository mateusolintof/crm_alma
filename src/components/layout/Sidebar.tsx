...
import { useToast, useUIStore } from '@/stores';
...
const SECONDARY_ITEMS = [{ label: 'Configurações', href: '/settings/pipelines', icon: Settings }];
      ...
      {!sidebarCollapsed && (
        <div className="px-3 py-3">
          <button
            ...
            className={clsx(
              'w-full flex items-center gap-3 px-3 py-2 rounded-lg focus-ring',
        ...
        <div className="px-2 py-3 flex justify-center">
          <Tooltip content="Buscar (⌘K)" side="right">
            <button
              onClick={handleCommandPalette}
              className={clsx(
                'w-10 h-10 flex items-center justify-center rounded-lg focus-ring',
        ...
        {NAV_ITEMS.map((item) => {
          ...
          const linkContent = (
            <Link
              href={item.href}
              className={clsx(
                'flex items-center gap-3 rounded-lg font-medium transition-all duration-150 focus-ring',
                sidebarCollapsed ? 'w-10 h-10 justify-center' : 'px-3 py-2.5',
                isActive
        ...
        {!sidebarCollapsed && (
          <div className="flex items-center gap-3 px-3 py-2 mb-2 rounded-lg bg-bg-hover">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-text-primary truncate">Administrador</p>
              <p className="text-xs text-text-tertiary truncate">admin@alma.agency</p>
            </div>
          </div>
        )}
